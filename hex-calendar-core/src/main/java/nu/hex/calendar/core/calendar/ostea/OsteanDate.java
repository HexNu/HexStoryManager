 ...
 * @author hl
 */
public class OsteanDate implements CalendarDate {

    private final int year;
    private final short month;
    private final short day;
    public static final CalendarDate MIN = OsteanDate.of(ErborigianYear.MIN_VALUE, 1, 1);
    public static final CalendarDate MAX = OsteanDate.of(ErborigianYear.MAX_VALUE, 12, 31);
    private static final int DAYS_PER_CYCLE = 146097;
    static final long DAYS_0000_TO_1970 = (DAYS_PER_CYCLE * 5L) - (30L * 365L + 7L);

    private OsteanDate(int year, int month, int day) {
        this.year = year;
        this.month = (short) month;
        this.day = (short) day;
    }

    private static OsteanDate create(int year, int month, int dayOfMonth) {
        if (dayOfMonth > 28) {
            int dom = 31;
            switch (month) {
                case 2:
                    dom = (ErborigianYear.of(year).isLeap() ? 29 : 28);
                    break;
                case 4:
                case 6:
                case 9:
        ...
        return new OsteanDate(year, month, dayOfMonth);

    }

    private static OsteanDate resolvePreviousValid(int year, int month, int day) {
        switch (month) {
            case 2:
                day = Math.min(day, ErborigianYear.of(year).isLeap() ? 29 : 28);
                break;
            case 4:
            case 6:
            case 9:
    ...
    public static OsteanDate of(int year, int moth, int dayOfMonth) {
        return create(year, moth, dayOfMonth);
    }

    public static OsteanDate ofYearDay(int year, int dayOfYear) {
        ...
        ErborigianMonth moy = ErborigianMonth.of((dayOfYear - 1) / 31 + 1);
        int monthEnd = moy.firstDayOfYear(leap) + moy.length(leap) - 1;
        if (dayOfYear > monthEnd) {
            moy = moy.plus(1);
        }
        int dom = dayOfYear - moy.firstDayOfYear(leap) + 1;
        return OsteanDate.of(year, moy.getValue(), dom);
    }
    ...
    public CalendarDayOfWeek getDayOfWeek() {
        ...
        return OsteanDayOfWeek.of(dow0 + 1);
    }
    ...
    public CalendarMonth getMonth() {
        return OsteanMonth.of((int) month);
    }
    ...
    public CalendarDate plusDays(long daysToAdd) {
        ...
        return OsteanDate.ofEpochDay(mjDay);
    }
    ...
    public static CalendarDate ofEpochDay(long epochDay) {
        ...
        int year = ErborigianYear.checkValidIntValue(yearEst);
    ...
    public static CalendarDate now() {
        ...
        return OsteanDate.parse(now.format(DateTimeFormatter.ISO_DATE), CalendarDateTimeFormatter.OSTEAN_DATE);
    }
